...
scraped_API = pd.DataFrame(np.nan, index = range(0,160000), columns = ['API', 'Interactant', 'Description'], dtype = str)
row_count = 0
...
for id in tqdm(range(0, HIGHEST_API_ID)):
    ...
    for n, m in zip(soup2.find_all('h5', {'class': 'interactant'}), soup2.find_all('div', {'class': 'span9 interaction-messages'})):
        API = soup2.find('h1', {'class': 'interaction-heading'}).span.getText().lstrip(' ')
