...
# This is a script to modify the Thesaurus dataframe (v2019) available by the IMthesaurusANSM R package.  
# Following consultation with the IMthesaurus ANSM R package developer (Cossin S.), the script recodes severity levels 
# from Thesaurus Rdata to capture the highest severity level associated with a DDI pair. 
...
### Method1
## code severity - most severe if several levels associated with a single DDI pair
...
regex <- "^[A-Za-z]+"
df_decompose$level <- stringr::str_extract(df_decompose$description_interaction,regex)
...
thesaurus_data <- df_decompose %>% arrange(desc(level)) %>% distinct(mol1, mol2, .keep_all = TRUE)

# Create an additional column by concatenating description_interaction and mecanisme columns
thesaurus_data$description_combined <- paste(thesaurus_data$description_interaction,thesaurus_data$mecanisme)

# Drop columns that we don't need and rename remaining columns
df_decompose_unique <- df_decompose_unique[-c(10,14)]
colnames(df_decompose_unique) <- c("interactant_1", "interactant_2","description_interaction","description_medoc","mechanism","CI","AD","PE","PC","drugname_1_original", "drugname_2_original","severity","description_blind", "description_combined")


# Keep only columns that we need and rename accordingly
thesaurus_data <- thesaurus_data[c(11,12,15,13)]
colnames(thesaurus_data) <- c("drugname_1_original", "drugname_2_original", "description_combined", "severity")
