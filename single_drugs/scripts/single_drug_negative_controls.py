...
cols = ['DRUG_CONCEPT_ID','EVENT_CONCEPT_ID','DRUG_CONCEPT_NAME', 'EVENT_CONCEPT_NAME']
single_drug_adr_bnf = pd.read_csv('event_mapping/output/bnf_single_mapped_with_eventnames.csv', 
                                  skiprows= 1, names = cols, dtype = str)
...
pos_ctls_df = single_drug_adr_df[single_drug_adr_df.duplicated(subset = cols, 
                    keep = False)].drop_duplicates(subset = cols)[cols].reset_index(drop=True)
...
for index, row in ae_text_df.iterrows():
    cur.execute("SELECT * FROM cdmv5.concept WHERE concept_id = (%s) \
        AND vocabulary_id = 'MedDRA' AND concept_class_id = 'PT' ;", [row['EVENT_CONCEPT_ID']])
    res = cur.fetchall()  
    if len(res) != 0:
...
with open('single_drugs/output/single_drug_negative_controls.csv', 'w', newline="") as f:
    writer = csv.writer(f)
    while i < no_of_controls:
        while True:
            try:
                ...
                # Check if the candidate negative control (i.e. drug-event pair) can be found in any of the two resources (i.e. BNF or SIDER) as an ADR
                lookup = single_drug_adr_df.loc[(single_drug_adr_df.DRUG_CONCEPT_NAME == d) & (single_drug_adr_df.EVENT_CONCEPT_NAME == pt)]
                # If not:
                if len(lookup) == 0:
                    # Submit a PubMed query
                    text_query = "(" + d + ") AND (" + pt + ') AND ((adverse event) OR (adverse drug reaction))'
