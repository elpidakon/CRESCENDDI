...
bnf.description_blinded as bnf_description, mi.description_blinded as micromedex_description, 
bnf.severity as bnf_severity, thes.severity as ansm_severity, mi.severity as micromedex_severity,
bnf.evidence as bnf_evidence, mi.evidence as micromedex_evidence,
lower(c1.concept_name) as drug_1_concept_name, lower(c2.concept_name) as drug_2_concept_name
...
update common
set bnf_description = trim(bnf_description), micromedex_description = trim(micromedex_description);
...
where ctid not in (
  select min(ctid)
  from common
  group by ordered_drug_list, bnf_description, micromedex_description
);
...
-- BNF description
COPY (select distinct bnf_description from common group by bnf_description) to 'specify_path/common_bnf_descriptions_blinded.csv' csv header;
-- Micromedex descriptions
COPY (select distinct micromedex_description from common group by micromedex_description) to 'specify_path/common_micromedex_descriptions_blinded.csv' csv header;
