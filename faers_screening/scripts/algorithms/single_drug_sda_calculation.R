...
# This script calculates scores of the signal detection algorithms for single drugs.
...
## You need to load the table standard_drug_ingr_outcome_count_tbl with drug-event FAERS counts AND the total no of FAERS reports (n..)

# A. MGPS and PRR

## Drug-Event pair FAERS counts
proc <- data.frame(var1=standard_drug_ingr_outcome_count_tbl$drug_concept_id,
...
ebout <- ebScores(proc, hyper_estimate = list(estimates = hyper_estimate),
                 quantiles = c(5, 95)) #For the 5th and 95th percentiles

# B. BCPNN
# Data table to be used as import
cont_tbl <- data.frame(var1 = standard_drug_ingr_outcome_count_tbl$drug_concept_id,
                       var2 = standard_drug_ingr_outcome_count_tbl$outcome_concept_id, 
...
DATABASE <- list()
DATABASE$L <- data.frame("Drug lab" = standard_drug_ingr_outcome_count_tbl$drug_concept_id,
                "AE lab" = standard_drug_ingr_outcome_count_tbl$outcome_concept_id)
DATABASE$data <- matrix(nrow = nrow(standard_drug_ingr_outcome_count_tbl), ncol = 3)

rownames(DATABASE$data) <- paste0(standard_drug_ingr_outcome_count_tbl$drug_concept_id, " ",
                       standard_drug_ingr_outcome_count_tbl$outcome_concept_id)
colnames(DATABASE$data) <- c("n11", "n1.", "n.1")

DATABASE$data[,1] <- as.numeric(standard_drug_ingr_outcome_count_tbl$drug_ingr_outcome_pair_count)
...
res <- PhViD::BCPNN(DATABASE,RR0 = 1, MIN.n11 = 1, DECISION = 1, DECISION.THRES = 0.05,
             RANKSTAT = 1, MC = FALSE, NB.MC = 10000)
res_2 <- PhViD::BCPNN(DATABASE,RR0 = 1, MIN.n11 = 1, DECISION = 1, DECISION.THRES = 0.05,
...
event_mappings <- read_excel('data_records/Data Record 5 - Event mappings v0.1 - PT mappings.xlsx')
...
single_counts_datafile = 'single_drugs/output/Data Record 3 - Single-drug ADRs, indications and negative controls.xlsx'
...
neg_ctls_tbl <-  neg_ctls_tbl %>% 
  merge(event_mappings, by.x = "EVENT_CONCEPT_NAME", 
...
# Convert to lowercase
intersect_pos_ctls_tbl$DRUG_CONCEPT_NAME <- tolower(intersect_pos_ctls_tbl$DRUG_CONCEPT_NAME)
intersect_pos_ctls_tbl$EVENT_CONCEPT_NAME <- tolower(intersect_pos_ctls_tbl$EVENT_CONCEPT_NAME)
...
intersect_ctls_tbl <-  intersect_ctls_tbl %>% 
  merge(ebout[["data"]], by.x = c("RXNORM_CODE/RXNORM_EXTENSION_CODE (OHDSI)", "PT_CONCEPT_ID"),
        by.y = c('var1','var2'), all.x = TRUE)

## Retrieve values from post_H0 and Q_0.025(log(IC)) (BCPNN) for union controls
intersect_ctls_tbl <- intersect_ctls_tbl %>% 
  merge(post_H0, by.x = c("RXNORM_CODE/RXNORM_EXTENSION_CODE (OHDSI)", "PT_CONCEPT_ID"),
        by.y = c("drug code","event effect"), all.x = TRUE)

intersect_ctls_tbl <- intersect_ctls_tbl %>% 
  merge(quant, by.x = c("RXNORM_CODE/RXNORM_EXTENSION_CODE (OHDSI)", "PT_CONCEPT_ID"),
        by.y = c("drug code","event effect"), all.x = TRUE)

# Write to a .csv file 
write.csv(intersect_ctls_tbl, 'faers_screening/output/single_drug_resource_union_scores.csv')
...
# Convert to lowercase
union_pos_ctls_tbl$DRUG_CONCEPT_NAME <- tolower(union_pos_ctls_tbl$DRUG_CONCEPT_NAME)
union_pos_ctls_tbl$EVENT_CONCEPT_NAME <- tolower(union_pos_ctls_tbl$EVENT_CONCEPT_NAME)
...
# Write to a .csv file 
write.csv(union_ctls_tbl, 'faers_screening/output/single_drug_resource_intersection_scores.csv')
