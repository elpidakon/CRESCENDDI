...
def omega_calculation(datafile, output):
    ...
    for index, row in df.iterrows():
        if 0 not in {row['d1_d2_counter'], row['not_d1_d2_counter'], row['d1_not_d2_counter'], row['n_111']}:
            ...
            g_11 = 1 - (1/(max(f_00/(1-f_00), f_10/(1-f_10))+max(f_00/(1-f_00), f_01/(1-f_01))-f_00/(1-f_00)+1))
...
omega_calculation('faers_screening/data/DR1_FAERS_COUNTS.xlsx', 'faers_screening/output/DR1_OMEGA_VALUES.xlsx')
omega_calculation('faers_screening/data/DR2_FAERS_COUNTS.xlsx', 'faers_screening/output/DR2_OMEGA_VALUES.xlsx')
