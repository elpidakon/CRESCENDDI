...
# This script calculates Interaction Signal Score (INtSS) values for the DDI
# controls.
...
## Drug-Event pair FAERS counts - You need to load the table
## standard_drug_ingr_outcome_count_tbl from FAERS
single_proc <- data.frame(
...
# Generate a df with drug-drug-event triplet counts from FAERS (this will be
# used as an input for hyperparameter estimation)
pair_proc <- data.frame(
  var1 = character(),
  var2 = character(),
  N = integer(),
  E = double(),
...
for (row in 1:nrow(ctls_tbl)) {
  ...
  n_1.. <- as.numeric(ctls_tbl[row, "d1_d2_counter"] +
    ctls_tbl[row, "d1_not_d2_counter"])
  n_.1. <- as.numeric(ctls_tbl[row, "d1_d2_counter"] +
    ctls_tbl[row, "not_d1_d2_counter"])
  n_..1 <- as.numeric(n_1.1 + ctls_tbl[row, "n_001"] + ctls_tbl[row, "n_011"])
  n_... <- as.numeric(n_1.. + ctls_tbl[row, "not_d1_d2_counter"] +
    ctls_tbl[row, "not_d1_not_d2_counter"])
  # Counts for drug pair
  pair_proc[row, ] <- list(
    paste(d1, d2, sep = "||", collapse = NULL),
    as.character(pt), n_111, (n_11. * n_..1) / n_...,
    (n_111 * n_...) / (n_11. * n_..1),
    (n_111 * (n_... - n_11.) / (n_11. * (n_..1 - n_111)))
  )
}
...
single_ebout <- ebScores(single_proc,
  hyper_estimate = list(estimates = hyper_estimate),
  quantiles = c(5, 95)
) # For the 5th and 95th percentiles
# EBGM scores for drug-drug-event triplets
pair_ebout <- ebScores(pair_proc,
  hyper_estimate = list(estimates = hyper_estimate),
  quantiles = c(5, 95)
) # For the 5th and 95th percentiles

## Generate a pair_df and use ebScores for single drugs by retrieving values
## from ebout in order to calculate the final IntSS
...
for (row in 1:nrow(pair_df)) {
  d1_quant_95 <- single_df %>%
    filter(data.var1 == pair_df[row, "d1"]) %>%
    filter(data.var2 == pair_df[row, "data.var2"]) %>%
    select("data.QUANT_95")

  d2_quant_95 <- single_df %>%
    filter(data.var1 == pair_df[row, "d2"]) %>%
    filter(data.var2 == pair_df[row, "data.var2"]) %>%
    select("data.QUANT_95")
  pair_df[row, "data.IntSS"] <-
    pair_df[row, "data.QUANT_05"] / max(d1_quant_95[1, ], d2_quant_95[1, ])
}


new_pair_df <- right_join(pair_df,
  ctls_tbl[, c(
    "no", "dde_tuple", "DRUG_1_CONCEPT_ID",
    "DRUG_2_CONCEPT_ID", "EVENT_CONCEPT_ID",
    "indicator"
  )],
  by = c(
    "d1" = "DRUG_1_CONCEPT_ID", "d2" = "DRUG_2_CONCEPT_ID",
    "data.var2" = "EVENT_CONCEPT_ID"
  )
)
new_pair_df <- unique(new_pair_df)

pos_ctls_intss <- new_pair_df %>%
  filter(indicator == 1) %>%
  select(no, dde_tuple, data.IntSS) %>%
  arrange(no)
neg_ctls_intss <- new_pair_df %>%
  filter(indicator == 0) %>%
  select(no, dde_tuple, data.IntSS) %>%
  arrange(no)
