...
select distinct
    i.*
  , lower(bnf.drugname_1_original) as bnf_drugname_original_first
  , lower(bnf.drugname_2_original) as bnf_drugname_original_second
  , bnf.description_blinded        as bnf_description
  , mi.interaction_effect_blinded  as micromedex_effect
...
set bnf_description   = trim(bnf_description)
  , micromedex_effect = trim(micromedex_effect)
;

-- Second effort for drug name blinding for salt forms etc that appeared 
-- in the description as ingredients (i.e. the drug names didn't match the names
...
--The output of this command can be exported in ordered to be used for 
-- mapping interaction effect descriptions to MedDRA concepts
-- BNF description
select distinct
    bnf_description
from
