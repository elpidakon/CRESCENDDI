...
# Script to generate negative controls for DDIs using drug and event concepts from the positive control set.
...
## A function to check if a dde tuple (i.e. drug-drug-event triplet) has a non-zero count in FAERS 
...
## Get two dictionaries with drugs and events that will be used to generate negative controls
pos_controls = pd.read_csv('ddi_controls/output/DR1_with_concept_ids.csv')

def create_dicts(dataframe):
    drug_sub_d_1 = dataframe.set_index('drug_1_concept_id')['drug_1_concept_name'].to_dict()
    drug_sub_d_2 = dataframe.set_index('drug_2_concept_id')['drug_2_concept_name'].to_dict()
...
## Find all drug pairs from any of the three DDI compendia (i.e. union of the three resources)
# BNF
cur.execute("select bnf.ordered_drug_list from drug_interaction_compendia.bnf_drug_pairs as bnf;")
bnf_drug_pair_list = [tup[0] for tup in cur.fetchall() if tup[0]]
bnf_drug_pair_tuples = [(int(i.split('|')[0]), int(i.split('|')[1])) for i in bnf_drug_pair_list]
# Micromedex
cur.execute("select mi.ordered_drug_list from drug_interaction_compendia.micromedex_drug_pairs as mi;")
micromedex_drug_pair_list = [tup[0] for tup in cur.fetchall() if tup[0]]
micromedex_drug_pair_tuples = [(int(i.split('|')[0]), int(i.split('|')[1])) for i in micromedex_drug_pair_list]
# Thesaurus
cur.execute("select thes.ordered_drug_list from drug_interaction_compendia.thesaurus_drug_pairs as thes;")
thesaurus_drug_pair_list = [tup[0] for tup in cur.fetchall() if tup[0]]
thesaurus_drug_pair_tuples = [(int(i.split('|')[0]), int(i.split('|')[1])) for i in thesaurus_drug_pair_list]

union_tuples = set(bnf_drug_pair_tuples + micromedex_drug_pair_tuples + thesaurus_drug_pair_tuples)
        ...
        while True:
            try:
                ...
                if dd not in union_tuples:
                    ...
                    text_query = "(" + drug_dict[dd[0]] + ") AND (" + drug_dict[dd[1]] + ') AND ((' + AE_dict[pt] + ') OR (interaction))'
                    # Submit the query
                    ...
                    if len(id_list) == 0:
                        # Check if the candidate negative control (i.e. dde tuple) has a non-zero count in FAERS
                        faers_triplet_count = is_in_faers(dd[0], dd[1], pt, drug_report_dict, pt_report_dict)
                        # If it does, then store the negative control
                        if faers_triplet_count == 'non-zero':
                            i += 1
                            writer.writerow([text_query, drug_dict[dd[0]], drug_dict[dd[1]], AE_dict[pt]])
            except IOError:
                continue
            break
...
negative_controls.columns = ['PubMed_query', 'DRUG_1_CONCEPT_NAME', 'DRUG_2_CONCEPT_NAME', 'EVENT_CONCEPT_NAME']

# Event name to MedDRA code dictionary
cur.execute("select c.concept_name, c.concept_code from cdmv5.concept c where c.vocabulary_id = 'MedDRA' and c.concept_class_id = 'PT';")
row = cur.fetchall() 
...
negative_controls['MDR_CODE'] = negative_controls['EVENT_CONCEPT_NAME'].map(event_name_to_code_dict)
