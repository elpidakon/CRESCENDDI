...
# This script maps drug names from SIDER ADR and Indication data to normalised 
# OHDSI concepts (RxNorm/RxNorm Extension Ingredients)
...
drug_names_df = pd.read_csv(
    "data_extraction/data/SIDER/drug_names.tsv",
    names=["STITCH_code", "drug_name"],
    sep="\t",
)
...
drug_atc_df = pd.read_csv(
    "data_extraction/data/SIDER/drug_atc.tsv",
    names=["STITCH_code", "ATC_code"],
    sep="\t",
)
...
# Drug side effects
drug_se = pd.read_csv(
    "data_extraction/data/SIDER/meddra_all_se.tsv",
    names=[
        "STITCH_code_1",
        "STITCH_code_2",
        "UMLS_code_label",
...
drug_indications = pd.read_csv(
    "data_extraction/data/SIDER/meddra_all_indications.tsv",
    names=[
        "STITCH_code",
        "UMLS_code_label",
        "detection_method",
...
drug_df = pd.merge(
    drug_names_df, drug_atc_df, left_index=True, right_index=True, how="outer"
).reset_index(drop=False)

# a. Exact string matching (Drug names to OHDSI RxNorm/RxNormExtension Ingredient concepts)

# A dictionary for mapping drug names to standardised OHDSI concepts 
# (RxNorm and RxNormExtension)
cur.execute(
    "SELECT concept_id, concept_name, concept_code FROM cdmv5.concept \
    WHERE vocabulary_id in ('RxNorm', 'RxNorm Extension') AND \
    concept_class_id = 'Ingredient' AND standard_concept = 'S';"
)
row_2 = cur.fetchall()
...
drug_df["exact_string_matching"] = np.where(
    drug_df["OHDSI_RxNorm_id_1"].isnull(), "No", "Yes"
)
drug_df.head()
...
for index, row in drug_df.iterrows():
    ...
    if type(row["ATC_code"]) != str:
        ...
        cur.execute(
            "SELECT concept_name FROM cdmv5.concept WHERE concept_code = (%s);",
            [row["ATC_code"]],
        )
...
for index, row in drug_df.iterrows():
    ...
    if type(row["ATC_code"]) != str:
        ...
        cur.execute(
            "SELECT cr.concept_id_2 FROM cdmv5.concept_relationship cr \
                    JOIN cdmv5.concept c ON cr.concept_id_1 = c.concept_id \
                    WHERE c.concept_code = (%s) AND cr.relationship_id = 'ATC - RxNorm';",
            [row["ATC_code"]],
        )
        drugname = cur.fetchall()
        if len(drugname) != 0:
...
# Convert the above dataframe into a final dictionary for mapping STITCH codes 
# to OHDSI RxNorm/RxNormExtension Ingredient concepts
stitch_to_rxnorm_id = {
    k: [v["OHDSI_RxNorm_id_1"], v["OHDSI_RxNorm_id_2"], v["OHDSI_RxNorm_id_3"]]
    for k, v in drug_df.iterrows()
}
# Remove any NaN values from the dictionary values
stitch_to_rxnorm_id.update(
    {k: [i for i in v if str(i) != "nan"] for k, v in stitch_to_rxnorm_id.items()}
)
...
# more mapped concepts.
# We will create a final version of the side effects table by 'exploding' rows 
# with multiple mapped drug concepts into separate rows
...
drug_indications["OHDSI_drug_id"] = drug_indications["STITCH_code"].map(
    stitch_to_rxnorm_id
)
# Similarly to the previous table for side effects, we will 'explode' rows with 
# multiple mapped drug concepts into separate rows
